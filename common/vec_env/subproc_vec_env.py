...
def worker(remote, map_name, nscripts, i):
    agent_format = sc2_env.AgentInterfaceFormat(feature_dimensions=sc2_env.Dimensions(screen=(32, 32), minimap=(32, 32)))
    with sc2_env.SC2Env(agent_interface_format=[agent_format], map_name=map_name, visualize=True, step_mul=2) as env:
        available_actions = []
        result = None
        group_list = []
        xy_per_marine = {}
            ...
            if cmd == 'step':
                ...
                if 331 in available_actions and move and not done:
                    try:
                        ...
                        print("e :", e)
                ob = (result[0].observation["feature_screen"][_PLAYER_RELATIVE:_PLAYER_RELATIVE + 1] == 3).astype(int)
                selected = result[0].observation["feature_screen"][_SELECTED:_SELECTED + 1]
                ...
                if len(action1[1]) == 2:
                    ...
                    player_y, player_x = (result[0].observation["feature_screen"][_SELECTED] == 1).nonzero()
                    if len(player_x) > 0:
                        if group_id == 1:
                ...
                ob = (time_step.observation["feature_screen"][_PLAYER_RELATIVE:_PLAYER_RELATIVE + 1] == 3).astype(int)
                selected = time_step.observation["feature_screen"][_SELECTED:_SELECTED + 1]
...
class SubprocVecEnv(VecEnv):
    def __init__(self, nenvs, nscripts, map_name):
        ...
        for (work_remote, ) in zip(self.work_remotes, ):
            self.ps.append(Process(target=worker, args=(work_remote, map_name, nscripts, i)))
            i += 1
        for p in self.ps:
            p.start()
    ...
    def step_wait(self):
        ...
        return (self._obs_from_buff(), np.copy(self.buf_rews), np.copy(self.buf_dones), self.buf_infos.copy())
