...
class Runner(object):
    ...
    def trans_base_action(self, base_actions):
        ...
        for n in range(self.nsteps):
            ...
            for env_num in range(self.nenv):
                ...
                if len(self.action_queue[env_num]) == 0:
                    self.action_queue[env_num], self.group_id[env_num], self.dest_per_marine[env_num], self.xy_per_marine[env_num] = common.solve_tsp(player_relative, self.selected[env_num][0], self.group_list[env_num], self.group_id[env_num], self.dest_per_marine[env_num], self.xy_per_marine[env_num])
            ...
            new_base_actions = self.trans_base_action(base_actions)
            ...
            for n, done in enumerate(dones):
                ...
                if done:
                    ...
                    if n < self.nscripts:
                        ...
                        mean_100ep_reward_script = round(np.mean(self.episode_rewards_script[-101:]), 1)
                        nsml.report(reward_script=self.total_reward[n], mean_reward_script=mean_100ep_reward_script, reward=self.total_reward[n], mean_100ep_reward=mean_100ep_reward, episodes=self.episodes, step=self.episodes, scope=locals())
                    else:
...
def learn(policy, env, seed, total_timesteps=int(40e6), gamma=0.99, log_interval=1, nprocs=24, nscripts=12, nsteps=20, nstack=4, ent_coef=0.01, vf_coef=0.5, vf_fisher_coef=1.0, lr=0.25, max_grad_norm=0.01, kfac_clip=0.001, save_interval=None, lrschedule='linear', callback=None):
    tf.reset_default_graph()
    set_global_seeds(seed)
    nenvs = nprocs
    ob_space = (32, 32, 3)
    ac_space = (32, 32)
    make_model = lambda: Model(policy, ob_space, ac_space, nenvs, total_timesteps, nprocs=nprocs,nscripts=nscripts, nsteps=nsteps, nstack=nstack, ent_coef=ent_coef, vf_coef=vf_coef, vf_fisher_coef=vf_fisher_coef, lr=lr, max_grad_norm=max_grad_norm, kfac_clip=kfac_clip, lrschedule=lrschedule)
...
def run(policy, env, seed, total_timesteps=int(40e6), gamma=0.99, log_interval=1, nprocs=24, nscripts=12, nsteps=20, nstack=4, ent_coef=0.01, vf_coef=0.5, vf_fisher_coef=1.0, lr=0.25, max_grad_norm=0.01, kfac_clip=0.001, save_interval=None, lrschedule='linear', callback=None):
    tf.reset_default_graph()
    set_global_seeds(seed)
    nenvs = nprocs
    ob_space = (32, 32, 3)
    ...
    make_model = lambda: Model(policy, ob_space, ac_space, nenvs, total_timesteps, nprocs=nprocs,nscripts=nscripts, nsteps=nsteps, nstack=nstack, ent_coef=ent_coef, vf_coef=vf_coef, vf_fisher_coef=vf_fisher_coef, lr=lr, max_grad_norm=max_grad_norm, kfac_clip=kfac_clip, lrschedule=lrschedule)
    model = make_model()
    model.load("models/a2c/mineral_115.1.pkl")
    print("make_model complete!")
    runner = Runner(env, model, nsteps=nsteps, nscripts=nscripts, nstack=nstack, gamma=gamma, callback=callback)
    nbatch = nenvs * nsteps
    for update in range(1, total_timesteps // nbatch + 1):
